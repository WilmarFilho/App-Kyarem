...
void main() {
  ...
  group('Teste da aba Partidas', () {
    testWidgets(
      'Deve exibir empty state para partidas e estatisticas caso vazias',
      (tester) async {
        when(
          () => mockPartidaService.getMatchesByModalityAndStatus(
        ...
        when(
          () => mockEstatisticaService.getEstatisticsByModality(any()),
        ).thenAnswer((_) async => []);
        ...
        expect(
          find.text('Nenhuma estatística encontrada para esta modalidade.'),
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets(
      'Deve renderizar lista de partidas, reagir a filtros e navegar',
      (tester) async {
  ...
  group('Teste da aba Estatísticas', () {
    testWidgets(
      'Renderiza lista de atletas com pódio se >= 3 atletas e gols ativo',
      (tester) async {
        when(
          () => mockPartidaService.getMatchesByModalityAndStatus(
            modalityId: any(named: 'modalityId'),
        ...
        expect(
          find.text('2º'),
          findsWidgets,
        ); // Um badge de cartao tbm tem "2º"? nao, strings hardcoded do pódio
