-- =============================================================================
-- V18 - Storage: bucket atleticas e políticas RLS
-- =============================================================================
-- Cria o bucket de escudos de atleticas no Supabase Storage e define
-- as políticas de Row Level Security (RLS) necessárias.
-- Diferente de avatares de usuário (que usam a UUID do usuário como pasta),
-- escudos de atleticas podem ser gerenciados livremente por usuários autenticados
...
DO $$ BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'storage'
          ...
          AND policyname = 'atleticas_public_read'
    ) THEN
        CREATE POLICY "atleticas_public_read" ON storage.objects
