    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(_screenTitle, style: const TextStyle(fontFamily: 'Bebas Neue', fontSize: 22, color: Colors.white, letterSpacing: 1)),
            Row(
              children: [
                Text(_screenSubtitle, style: const TextStyle(fontSize: 12, color: Colors.white70, fontWeight: FontWeight.normal)),
                if (!widget.canEdit) ...[
                  ...
                  Container(
                    ...
                    child: const Text('👁 Leitura', style: TextStyle(fontSize: 10, color: Colors.white)),
                  ),
                ],
              ],
            ),
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh, color: Colors.white), onPressed: _carregarPartidas),
        ],
      ),
      ...
      floatingActionButton: widget.canEdit
          ? FloatingActionButton.extended(
              ...
              label: const Text('Nova Partida', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            )
          : null,
      body: Stack(
        children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 6),
                          Text(_statusLabel(p.status), style: TextStyle(fontSize: 12, color: statusColor, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                    if (modNome.isNotEmpty) ...[
                      const SizedBox(width: 8),
                      Text(modNome, style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                    ],
                    const Spacer(),
                ...
                Row(
                  children: [
                    Expanded(
                      child: Column(
                        children: [
                          _buildEscudo(p.equipeA?.atleticaEscudoUrl, statusColor),
                          const SizedBox(height: 6),
                          Text(nomeA, textAlign: TextAlign.center,
                              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
                              maxLines: 2, overflow: TextOverflow.ellipsis),
                          // Atlética do time A
                          if (p.equipeA?.atletica?.nome != null)
                            Text(p.equipeA!.atletica!.nome,
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          _buildEscudo(p.equipeB?.atleticaEscudoUrl, Colors.grey.shade400),
                          const SizedBox(height: 6),
                          Text(nomeB, textAlign: TextAlign.center,
                              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
                              maxLines: 2, overflow: TextOverflow.ellipsis),
                          if (p.equipeB?.atletica?.nome != null)
                            Text(p.equipeB!.atletica!.nome,
                                style: TextStyle(fontSize: 11, color: Colors.grey.shade500),
                                textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
                        ],
                      ),
                ...
                if (p.local != null && p.local!.isNotEmpty) ...[
                  ...
                  Row(
                    children: [
                      ...
                      Text(p.local!, style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                    ],
                  ),
                ],
                if (p.fase != null && p.fase!.isNotEmpty) ...[
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 4),
                      Text(p.fase!, style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                    ],
                  ),
                ],
              ],
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(Icons.sports_soccer_outlined, size: 72,
          ...
          Text(
            widget.canEdit ? 'Toque em "Nova Partida" para criar' : 'Nenhuma partida agendada ainda',
